#+Title: Responsive Web Design Certificate Notes
#+Author: MeCantSpell
...
readers:
1. *Windows*: Windows Narrator (can be accessed by using the shortcut _WIN+CTRL+ENTER_)
2. *iOS & MacOS*: VoiceOver (on MacOS can be accessed via _CMD+F5_ and for iOS via _settings_)
3. *Linux*: Orca for desktop environments and Speakup for terminal.
4. *Android*: Talkback, Ella and Select to Speak. (can be accessed via _settings_)
...
1. *Windows*: Magnifier (can be accessed via _settings -> Ease of Access -> Magnifier_ or by _simply_
   _typing "magnifier" in the Windows search bar_)
2. *iOS & MacOS*: Zoom (can be accessed via _settings_)
3. *Android*: Magnification (can be accessed via _settings -> special function -> accessibility_
   _->magnification_)
...
*Google Lighthouse*: This is already built into your DevTools which can be opened using the _F12_ key.
...
** ARIA
When we looked at the [[Fundamentals of Accessibility]] we learnt about WCAG which provided guidelines
...
12. Input - <input/>
    ...
    g. Add a placeholder value in your input field - placeholder
    h. Minimum characters that have to be input - minlength
    i. Maximum characters that have to be input - maxlength
    j. The focused state of the input - focused
    k. Size of the input box - size
    l. Have a checkbox/radio already checked - checked 
13. Italicize - <i>,</i>
...
29. Audio and video text tracks - <track/>
    ...
    c. Language - srclang
    d. Language Label - label
    e. Enable the track by default - default
30. Inline Frame used to embed content such as videos, maps and documents - <iframe>,</iframe>
    a. Source - src
    b. HTML Source - srcdoc
    c. Width - width
    d. Height - height
    e. Title which works like alt-text - title
    f. Allowing what the iframe can do - allow 
31. SVG - <svg>,</svg>
    a. Width - width
    b. Height - height
    c. How much the browser will display - viewbox
32. SVG Circles - <circle/>
    a. Border - stroke
    b. Border width - stroke-width
    c. Colour fill - fill
33. SVG Paths - <path/>
    a. Path colour - stroke
    b. Path width - stroke-width
    c. Area between path colour - fill	  
...
36. Forms - <form>,</form>
    ...
    a. Where form data will be sent - action
    b. Method in which data will be sent (by default it is get) - method
...
38. Button - <button>,</button>
    ...
    functionality.
    a. Set the type of button - type
       You can set the type to be either a submit or reset button.
39. Text area for long user input - <textarea>,</textarea>
    a. Set the number of columns - cols
    b. Set the number of rows - rows
40. Dropdown - <select>,</select>
    a. Scrolling List - multiple size
41. Dropdown options - <option>,</option>
    a. Value of the option - value
    b. Have a preselected option - selected
...
48. Table Data - <td>,</td>
    a. Span over multiple columns - colspan
...
** CSS Syntax
1. Anchor Elements
   a. Default link state - :link
   b. Visited links - :visited
   c. Links being hovered upon - :hover
   d. Links being focused upon - :focus
      I personally cannot tell the difference between :focus and :active
   e. Links that are being actively interacted with - :active
...
2. Text Elements
   a. Set the text alignment - text-align:
   ...
   b. Set the margin of the element - margin:
...
3. Gradients
   ...
   c. Conic Gradients - conic-gradients:

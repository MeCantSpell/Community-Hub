...
\label{sec:org7181872}
Earlier in \ref{sec:org77910b0} we looked at files like .html, .css and .js that we
...
\label{sec:orgbc160b6}
\subsubsection{Fundamentals of Accessibility}
\label{sec:org82671c9}
...
\begin{enumerate}
\item \textbf{Perceivable}: The content must be perceivable by all users.
\item \textbf{Operable}: The content must be operable by all users.
For example, some users may not be able to use a mouse and may need to navigate your webpage
using only the keyboard.
\item \textbf{Understandable}: The content must be easily understandable by all users.
\item \textbf{Robust}: The content must be interpreted by a wide range of tools including browsers and other
tools that may be used to help with accessibility (for example: a screen reader)
\end{enumerate}
These 4 core principles can be remembered using the acronymn: "\textbf{POUR}"
\subsubsection{Screen Readers}
\label{sec:orgb61944b}
When it comes to the topic of accessibility we mentioned earlier that there are certain tools that
can be used to "\textbf{O}-perate" the web. One of the more infamous ones is the screen-reader. While it's
...
readers:
\begin{enumerate}
\item \textbf{Windows}: Windows Narrator (can be accessed by using the shortcut \uline{WIN+CTRL+ENTER})
\item \textbf{iOS \& MacOS}: VoiceOver (on MacOS can be accessed via \uline{CMD+F5} and for iOS via \uline{settings})
\item \textbf{Linux}: Orca for desktop environments and Speakup for terminal.
\item \textbf{Android}: Talkback, Ella and Select to Speak. (can be accessed via \uline{settings})
\end{enumerate}
...
\subsubsection{Screen Magnifiers}
\label{sec:org89474b4}
...
\begin{enumerate}
\item \textbf{Windows}: Magnifier (can be accessed via \uline{settings -> Ease of Access -> Magnifier} or by \uline{simply}
\uline{typing "magnifier" in the Windows search bar})
\item \textbf{iOS \& MacOS}: Zoom (can be accessed via \uline{settings})
\item \textbf{Android}: Magnification (can be accessed via \uline{settings -> special function -> accessibility}
\uline{->magnification})
\item \textbf{Linux}: Zoom or Magnifier
...
\begin{enumerate}
\item Using scalable fonts
\item Creating an adaptive user interface using responsive web design
\item Using high contrast schemes and customizable colors
\item Implementing a non-sticky and tiny navbar
\item Using HTML text instead of images that have text.
\item Providing feedback directly next to the element that triggers it.
\end{enumerate}
\subsubsection{Voice Recognition Software}
\label{sec:org9f7216e}
...
\begin{enumerate}
\item \textbf{Windows}: Voice Access
\item \textbf{Android}: Voice Access
\item \textbf{iOS/MacOS}: Voice Control
\end{enumerate}
There is also Dragon Naturally Speaking by Nuance that is a very popular third party software.
\subsubsection{Accessibility Auditing}
\label{sec:orgde35140}
...
\textbf{Google Lighthouse}: This is already built into your DevTools which can be opened using the \uline{F12} key.
\textbf{WAVE}: This is a free extension available for Chromium and FireFox Browsers
\textbf{IBM Equal Accessibility Checker}: Another robust tool available as an extension for Chromium and
FireFox browsers. This tool can also generate an accessibility report for you that can be a
spreadsheet or HTML doc.
\subsection{ARIA}
\label{sec:org0d3ece1}
When we looked at the \ref{sec:org82671c9} we learnt about WCAG which provided guidelines
...
\subsubsection{ARIA Roles}
\label{sec:org410bf00}
...
\begin{enumerate}
\item \textbf{Document Structure Roles}: Roles that define the overall structure of the webpage, most of these
...
\item \textbf{Widget Roles}: Roles that define the purpose and functionality of interactive widgets such as:
...
\item \textbf{Landmark Roles}: Landmark roles label the primary sections of a webpage. Almost all landmark roles
...
\item \textbf{Live Region Roles}: These are roles that define elements that are being changed live (dynamic
...
\item \textbf{Window Roles}: These are roles that define sub-windows such as pop-up modal dialogues. Examples
...
\item \textbf{Abstract Roles}: These are roles that the browsers use to organize the document. These are not
used by developers.
\end{enumerate}
\subsubsection{ARIA Properties}
\label{sec:orgf06d760}
...
\begin{enumerate}
\item \textbf{aria-label}: Used to create a text label that can identify the element.
\item \textbf{aria-labelledby}: Used to create a text label that can identify the element by referencing text
...
\item \textbf{aria-describedby}: Used to add additional context to an element. Such as a warning message or
instruction that may provide vital information to using the element in a productive manner.
\end{enumerate}
\subsubsection{ARIA States}
\label{sec:org468dffa}
...
\item Add a placeholder value in your input field - placeholder
\item Minimum characters that have to be input - minlength
\item Maximum characters that have to be input - maxlength
\item The focused state of the input - focused
\item Size of the input box - size
\item Have a checkbox/radio already checked - checked
\end{enumerate}
\item Italicize - <i>,</i>
...
\begin{enumerate}
\item Where form data will be sent - action
\item Method in which data will be sent (by default it is get) - method
\end{enumerate}
...
functionality.
\begin{enumerate}
\item Set the type of button - type
You can set the type to be either a submit or reset button.
\end{enumerate}
\item Text area for long user input - <textarea>,</textarea>
\begin{enumerate}
\item Set the number of columns - cols
\item Set the number of rows - rows
\end{enumerate}
\item Dropdown - <select>,</select>
\begin{enumerate}
\item Scrolling List - multiple size
\end{enumerate}
\item Dropdown options - <option>,</option>
\begin{enumerate}
\item Value of the option - value
\item Have a preselected option - selected
\end{enumerate}
\item Tables - <table>,</table>
\item Table Heading - <thead>,</thead>
\item Table Body (Actual Table Content) - <tbody>,</tbody>
\item Table Footer - <tfoot>,</tfoot>
\item Table Row - <tr>,</tr>
\item Table Row Header - <th>,</th>
\item Table Data - <td>,</td>
\begin{enumerate}
\item Span over multiple columns - colspan
\end{enumerate}
\item Table Caption - <caption>,</caption>
\end{enumerate}
\section{CSS}
\label{sec:org98792c3}

\subsection{CSS Syntax}
\label{sec:org5bf22f5}
\begin{enumerate}
...
\end{verbatim}
This can be easily rememebered using the mnemonic "\textbf{LoVe Hates FeAr}". It must also be noted that
these are not CSS properties but rather pseudo-classes that mimic both classes as well as properties.
\item Text Elements
\begin{enumerate}
\item Set the text alignment - text-align:
Text alignment can be said to several different types:
\begin{itemize}
\item start - aligns text depending on the start direction of the letters used.
\item end - aligns text depending on the end direction of the letters used.
\item left - aligns text to the left.
\item right - aligns text to the right.
\item center - aligns text to the middle.
\item justify - justifies the text.
\item match-parent - aligns the text as the parent element of the element is aligned.
\end{itemize}
\item Set the margin of the element - margin:
...
\begin{enumerate}
\item Linear Gradients - linear-gradient:
Linear gradients can be set up with many properties
\begin{itemize}
\item colours - linear gradients can be set up with colours including web colors, RGB Hexadecimal
Notation, RGB Decimal Notation
\item direction -  direction can be indicated in many ways inlcuding the literal direction using
...
\item angle - another way is to describe the angle from which the gradient will start and end
\item colour interpolation - I don't know what that means, but you can interpolate colours in
...
\item colour stop - You can indicate where a colour should stop and start, this can be done by
...
\end{verbatim}
\item Radial Gradients - radial-gradient:
Radial gradients follow the same rules for colours and interpolation as linear-gradients, other than that:
\begin{itemize}
\item shape - A radial gradient can be specified to be either a circle gradient or an ellipse
gradient. If no shape is given the gradient defaults to ellipse.
\item position - A position can be selected where the gradient begins.
\end{itemize}
\item Conic Gradients - conic-gradients:
